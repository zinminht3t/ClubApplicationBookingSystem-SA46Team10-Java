...
public class ReportController {
	...
	/*
	 * @RequestMapping(value = "/reports", method = RequestMethod.GET) public int
	 * maleNum() { int a = rService.findNumOfMales(); return a; }
	 */
	...
	public void getRpt1(HttpServletResponse response) throws IOException, JRException, SQLException {
		...
		// To obtain the file stream thing. The jrxml file has to be compiled to a
		// jasper file (do it in Spring itself)
		...
		Connection connection = DriverManager
				.getConnection("jdbc:mysql://localhost/team12cabproject?user=root&password=password");
		...
		// this starts the process to be able to generate a pdf report straight from the
		// webpage
		response.setContentType("application/x-pdf");
		response.setHeader("Content-disposition", "inline; filename=memberList.pdf");
	...
	public void getRpt2(HttpServletResponse response) throws IOException, JRException, SQLException {
		...
		InputStream jasperStream = this.getClass().getResourceAsStream("/jasperReports/facilRevReport.jasper");
		Map<String, Object> params = new HashMap<>();
		Connection connection = DriverManager
				.getConnection("jdbc:mysql://localhost/team12cabproject?user=root&password=password");
		JasperReport jasperReport = (JasperReport) JRLoader.loadObject(jasperStream);
		...
		response.setContentType("application/x-pdf");
		response.setHeader("Content-disposition", "inline; filename=facilRevReport.pdf");
	...
	public void getRpt3(HttpServletResponse response) throws IOException, JRException, SQLException {
		...
		InputStream jasperStream = this.getClass().getResourceAsStream("/jasperReports/facilUseReport.jasper");
		Map<String, Object> params = new HashMap<>();
		Connection connection = DriverManager
				.getConnection("jdbc:mysql://localhost/team12cabproject?user=root&password=password");
		JasperReport jasperReport = (JasperReport) JRLoader.loadObject(jasperStream);
		...
		response.setContentType("application/x-pdf");
		response.setHeader("Content-disposition", "inline; filename=facilUseReport.pdf");

		OutputStream outStream = response.getOutputStream();
		JasperExportManager.exportReportToPdfStream(jasperPrint, outStream);
	}

	@RequestMapping(value = "/reports/report4", method = RequestMethod.GET)
	@ResponseBody
	public void getRpt4(HttpServletResponse response) throws IOException, JRException, SQLException {

		InputStream jasperStream = this.getClass().getResourceAsStream("/jasperReports/maintenanceReport.jasper");
		Map<String, Object> params = new HashMap<>();
		Connection connection = DriverManager
				.getConnection("jdbc:mysql://localhost/team12cabproject?user=root&password=password");
		JasperReport jasperReport = (JasperReport) JRLoader.loadObject(jasperStream);
